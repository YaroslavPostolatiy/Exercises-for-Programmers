...
# FIXME: we use snake_case in python instead of camelCamelCase. So turnTheKey -> turn_the_key
turnTheKey = str(
    input("Is the car silent when you turn the key? Press Y if so or N for no.").upper()
)
if turnTheKey == "Y":
    batteryTerminalsCorroded = str(
...
if turnTheKey == "N":
    ...
    if carMake == "N":
        arcCrankUp = str(
            input(
                "Does the car crank up but fail to start? Press Y if so or N for no."
            ).upper()
        )
        if arcCrankUp == "Y":
        ...
        if arcCrankUp == "N":
            engineStert = str(
                input(
                    "Does the engine start and then die? Press Y if so or N for no."
                ).upper()
            )
            if engineStert == "Y":
                fuel = str(
